function rst = SHAMODE(fun,fout,nloop,nsol,nvar,nbit,narchive,a,b)
...
%         pause(1)
        ...
        [adapt_dat]=DE_adaptive_processor(adapt_dat,x0,f0,g0,nsort);
...
function [x1,f1,g1,adapt_dat] = de_reproduct(fun,x0,f0,g0,ppareto,fpareto,gpareto,a,b,iter,nloop,adapt_dat)
    ...
    % Bound and Discrete truncated
%     [xm,~,~,~]=var_truncate(xm,a,b);
...
function [adapt_dat] = DE_adaptive_processor(adapt_dat,x0,f0,g0,nsort)
    % Get success update index (Parent of success offspring)
    % index 1:nsol = Parent, (nsol+1):(2*nsol) = Offspring
    sind=nsort(1:100); % Success Offspring = Offspring in first 1:nsol rank
    sind=sind(sind>100)-100; % Offspring index - nsol = Parent index
    %
    nsol=size(x0,1);
